...
import { ChevronLeft, ChevronRight, Download, FileWarning, Maximize2, Minus, Plus, RefreshCw } from "lucide-react";
...
export function PdfPreview({ compact = false }: { compact?: boolean }) {
  ...
  const filename = sanitizeFilename(`${workspace.resume.basics.name}-${workspace.targetJob?.company ?? "resume"}`);
  const controls = <div className="flex items-center gap-1"><Button size="icon" variant="ghost" onClick={() => setZoom((value) => Math.max(0.4, value - 0.1))} aria-label="Zoom out"><Minus /></Button><span className="w-12 text-center font-mono text-[10px] text-muted-foreground">{Math.round(zoom * 100)}%</span><Button size="icon" variant="ghost" onClick={() => setZoom((value) => Math.min(1.8, value + 0.1))} aria-label="Zoom in"><Plus /></Button><Button size="icon" variant="ghost" onClick={() => setZoom(0.82)} aria-label="Fit page"><Maximize2 /></Button></div>;
...
function PreviewMessage({ label }: { label: string }) { return <div className="grid h-[720px] w-[556px] place-items-center bg-white text-xs text-zinc-500">{label}</div>; }

function BlobPdf({ blob, page, zoom, onLoaded }: { blob: Blob; page: number; zoom: number; onLoaded: (pages: number) => void }) {
  ...
  if (!fileProp) {
    return <div className="mx-auto w-fit shadow-2xl shadow-black/25"><PreviewMessage label="Loading PDF…" /></div>;
  }
