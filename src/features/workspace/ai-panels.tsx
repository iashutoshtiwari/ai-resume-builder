...
import { AlertTriangle, Check, CircleDashed, RefreshCw, ShieldAlert, Sparkles, X } from "lucide-react";
...
export function JobPanel({ aiConfigured }: { aiConfigured: boolean }) {
  ...
  return <div><PanelHeading eyebrow="Target / Job" title="Evidence coverage" copy="Coverage is explainable; there is no invented ATS score." action={workspace.jobAnalysis && <Button size="sm" disabled={busy !== null || !aiConfigured} onClick={() => void tailor()}>{busy === "tailor" ? <CircleDashed className="animate-spin" /> : <Sparkles />} Generate changes</Button>} />
    <div className="space-y-5 p-5">
      ...
      {!aiConfigured && <div className="border-l-2 border-warning bg-warning/5 px-4 py-3 text-sm text-muted-foreground">AI is disabled because no server key is configured. Editing, local compilation, and export still work.</div>}
      ...
      <p className="border-t border-border pt-4 text-xs leading-5 text-muted-foreground">AI requests are processed directly via Google AI Studio Gemini API with structured outputs.</p>
    </div>
  </div>;
}
...
function ChangeCard({ change }: { change: ResumeChange }) {
  ...
  return <article className="border border-border bg-card"><div className="flex items-center justify-between border-b border-border px-4 py-3"><div className="flex items-center gap-2"><span className="font-mono text-[10px] uppercase tracking-wider text-muted-foreground">{change.type.replace("-", " ")}</span><Badge variant={change.risk === "safe" ? "secondary" : "outline"}>{change.risk}</Badge></div>{statusBadge(change.status, stale)}</div><div className="space-y-4 p-4">{change.type === "rewrite-text" && (editing ? <Textarea value={edited} onChange={(event) => setEdited(event.target.value)} className="min-h-24" /> : <WordDiff before={change.before} after={change.after} />)}{change.type === "remove-item" && <WordDiff before={change.before} after="" />}{change.type === "reorder-item" && <p className="text-sm text-muted-foreground">Move this item to the proposed position.</p>}<div><p className="text-xs font-medium">Why</p><p className="mt-1 text-xs leading-5 text-muted-foreground">{change.reason}</p></div><div className="flex flex-wrap items-center gap-2">{editing ? <><Button size="sm" disabled={stale} onClick={() => apply(edited)}>Apply edited</Button><Button size="sm" variant="ghost" onClick={() => setEditing(false)}>Cancel</Button></> : <><Button size="sm" disabled={stale || change.status !== "pending"} onClick={() => apply()}>Accept</Button>{change.type === "rewrite-text" && <Button size="sm" variant="outline" disabled={stale || change.status !== "pending"} onClick={() => setEditing(true)}>Edit & accept</Button>}<Button size="sm" variant="ghost" disabled={change.status !== "pending"} onClick={() => setStatus(change.id, "rejected")}><X /> Reject</Button></>}</div></div></article>;
}
