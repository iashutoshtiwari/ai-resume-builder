...
export async function POST(request: Request) {
  try {
    ...
    const key = await requestKey("analyze-job", { ...input, guidance });
    const result = await dedupeRequest(key, () => getResumeAIProvider().analyzeJob(input.resume, input.targetJob, guidance));
